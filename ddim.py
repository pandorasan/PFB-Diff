...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            self.score_correct(e_t, img, ts, cond, score_corrector, corrector_kwargs)
    ...
    def score_correct(self, e_t, x, t, context, score_corrector=None, corrector_kwargs=None):
        if score_corrector is not None:
            ...
            e_t = score_corrector.modify_score(self.model, e_t, x, t, context, **corrector_kwargs)
        return e_t
    ...
    def ddim_loop(self, latent, cond_embeddings, t_enc):
        ...
        for i, step in enumerate(iterator):
            ts = torch.full((latent.shape[0],), step, device=latent.device, dtype=torch.long)
            # print(ts, step)  # 1, 21, 41....981
            noise_pred, _ = self.model.apply_model(latent, ts, cond_embeddings)
            latent = self.next_step(noise_pred, step, latent, use_original_steps=True)
    ...
    def next_step(self, e_t, timestep, x, use_original_steps=False, quantize_denoised=False):
        ...
        sqrt_one_minus_a_t = torch.full((b, 1, 1, 1), sqrt_one_minus_alphas[timestep], device=device)
    ...
    def get_noise_pred_single(self, x, c, t, unconditional_conditioning=None, unconditional_guidance_scale=1.):
        ...
        c_in = torch.cat([unconditional_conditioning, c])  # c_in.shape=(2, 77, 768)
    ...
    def stochastic_encode(self, x0, t, use_original_steps=False, noise=None):
        ...
        # print(sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod)
    ...
    def ddim_inverse(self, latent, context, t_start, unconditional_guidance_scale=1.0,
        ...
        iterator, total_steps = self.prepare_iterator(t_start, use_original_steps)
        ...
        for i, step in enumerate(iterator):
            ...
            if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                ...
                e_t = self.get_noise_pred_single(x, context, ts,
                                                 unconditional_guidance_scale=unconditional_guidance_scale,
                                                 unconditional_conditioning=unconditional_conditioning)
            x = self.prev_step(x, e_t, index, use_original_steps=use_original_steps)
        return x
    ...
    #                                              unconditional_guidance_scale=unconditional_guidance_scale,
    #                                              unconditional_conditioning=unconditional_conditioning)
    #         x_dec = self.prev_step(x_dec, e_t, index, use_original_steps=use_original_steps)
    #
    #     return x_dec
    ...
    def replace_object_single_step(self, x, x_ref, t, src, dst, index, unconditional_conditioning,
        ...
        # x_pred_ref = self.prev_step(x_ref, e_ref, index, use_original_steps=use_original_steps)
        if t > model_kwargs['t_combine']:
            ...
            e_t_uncond = self.model.apply_model(x, t, unconditional_conditioning)[0]
            e_t = self.model.apply_model(x, t, dst)[0]

            # e_t_uncond = self.model.apply_model(x, t, unconditional_conditioning)[0]
            # e_t = self.model.apply_model(x, t, dst)[0]
        # grad = e_ref * (1 - model_kwargs['tgt_mask']) + e_t * model_kwargs['tgt_mask'] - e_t_uncond
        grad = e_t - e_t_uncond
        e_t = e_t_uncond + unconditional_guidance_scale * grad
    ...
    @torch.no_grad()
    def ddim_style_transfer(self, latent, ref_latent, src_cond, tgt_cond, t_start,
                            unconditional_guidance_scale=1.0,
                            unconditional_conditioning=None, use_original_steps=False,
                            corrector_fn=None,
                            quantize_denoised=False,
        ...
        for i, step in enumerate(iterator):
            ...
            x, x_ref = self.style_transfer_single_step(x, x_ref, ts, src_cond, tgt_cond, index,
                                                       unconditional_conditioning,
                                                       unconditional_guidance_scale, use_original_steps,
                                                       quantize_denoised=quantize_denoised,
                                                       temperature=temperature, noise_dropout=noise_dropout,
    ...
    @torch.no_grad()
    def style_transfer_single_step(self, x, x_ref, t, src, dst, index, unconditional_conditioning,
                                   unconditional_guidance_scale, use_original_steps,
                                   quantize_denoised=False,
                                   temperature=1.,
                                   noise_dropout=0.,
        ...
        _, h_ref = self.model.apply_model(x_ref.clone(), t, src)
        e_ref = self.get_noise_pred_single(x_ref, src, t,
                                           unconditional_conditioning,
                                           7.5)

        self.score_correct(e_ref, x_ref, t, src, score_corrector, corrector_kwargs)
        ...
        return x_pred, x_pred_ref

    def replace_from_start(self, S, latent, ref_latent, src_cond, tgt_cond, t_start,
                           unconditional_guidance_scale=1.0, eta=0.0, verbose=True,
                           unconditional_conditioning=None, use_original_steps=False,
                           tgt_mask=None, corrector_fn=None, fusion_layers=None,
                           quantize_denoised=False,
                           temperature=1.,
                           noise_dropout=0.,
                           score_corrector=None,
        ...
        for i, step in enumerate(iterator):
            ...
            x = self.replace_object_single_step(x, x_ref, ts, src_cond, tgt_cond, unconditional_conditioning,
                                                unconditional_guidance_scale, tgt_mask, fusion_layers,
                                                attn_mask=attn_mask,
                                                t_combine=t_combine, quantize_denoised=quantize_denoised,
                                                temperature=temperature, noise_dropout=noise_dropout,
                                                score_corrector=score_corrector,
                                                corrector_kwargs=corrector_kwargs
                                                )

            e_ref = self.get_noise_pred_single(x_ref, src_cond, ts, unconditional_conditioning,
                                               unconditional_guidance_scale)
            self.score_correct(e_ref, x_ref, ts, src_cond, score_corrector, corrector_kwargs)

            x_ref = self.prev_step(x_ref, e_ref, index, use_original_steps=use_original_steps,
                                   quantize_denoised=quantize_denoised,
                                   temperature=temperature, noise_dropout=noise_dropout)

            if corrector_fn is not None:
                x = corrector_fn(x, x_ref, index)
        return x, x_ref

    # def replace_from_start(self, S, latent, ref_latent, src_cond, tgt_cond, t_start,
    #                        unconditional_guidance_scale=1.0, eta=0.0, verbose=True,
    #                        unconditional_conditioning=None, use_original_steps=False,
    #                        label_mask=None, corrector_fn=None, fusion_layers=None,
    #                        quantize_denoised=False,
    #                        temperature=1.,
    #                        noise_dropout=0.,
    #                        score_corrector=None,
    ...
    #         if step > t_combine:
    #
    #             e_t = self.replace_object_single_step(x, x_ref, ts, src_cond, tgt_cond, unconditional_conditioning,
    #                                       unconditional_guidance_scale, label_mask, fusion_layers)
    #         else:
    #             e_t = self.get_noise_pred_single(x, tgt_cond, ts, unconditional_conditioning,
    #                                              unconditional_guidance_scale)
    #
    #         self.score_correct(e_t, x, ts, tgt_cond, score_corrector, corrector_kwargs)
    #
    #         e_ref = self.get_noise_pred_single(x_ref, src_cond, ts, unconditional_conditioning,
    #                                            unconditional_guidance_scale)
    #         self.score_correct(e_ref, x_ref, ts, src_cond, score_corrector, corrector_kwargs)
