...
def get_coco_mask_dilate(name, root_dir, dilate_args=None):
    ...
    # cv2.imwrite('00000.png', img_mask * 255)
...
def predict_mask(sampler, model, src, dst, uc, init_latent, noised_sample_num, ddim_steps, encode_ratio=0.5,
    ...
    noised = sampler.stochastic_encode(repeated, torch.tensor([t_enc] * 1).to(device))
...
def baseline(sampler, model, tgt_img, tgt_mask,
             src_prompt,
             dst_prompt,
             encode_ratio,
             ddim_steps,
    ...
    init_latent = model.get_first_stage_encoding(model.encode_first_stage(tgt_img))

    if tgt_mask is None:
        print('Diffedit mask generation')
        tgt_mask = predict_mask(sampler, model, src_cond, tgt_cond, uc, init_latent, 3, ddim_steps)

    # Image.fromarray(np.uint8(tgt_mask.cpu().numpy() * 255)).save('ooo.png')
    ...
    if deterministic:
        ...
        def corrector_fn(x, index):
            stochastic_intermediate = sampler.stochastic_encode(init_latent, index)
            x = x * tgt_mask + (1 - tgt_mask) * stochastic_intermediate
            return x
...
def inverse_image(sampler, model, init_image,
                  src_prompt,
                  encode_ratio,
                  ddim_steps,
                  scale: float = 7.5,
    ...
    if scale != 1.0:
        uc = model.get_learned_conditioning([""])
    context = model.get_learned_conditioning([src_prompt])
    init_latent = model.get_first_stage_encoding(model.encode_first_stage(init_image))

    if mode == 'deterministic':
        t_enc = int(ddim_steps * encode_ratio)

        all_latents = sampler.ddim_loop(init_latent, context, t_enc)

        # decode it
        recover_latent = sampler.ddim_inverse(all_latents[-1], context, t_enc,
                                              unconditional_guidance_scale=scale,
                                              unconditional_conditioning=uc)
        ...
        t_enc = int(ddim_steps * encode_ratio)
        latent = sampler.stochastic_encode(init_latent,
                                           torch.tensor([min(t_enc, ddim_steps - 1)] * 1).to(device))

        recover_latent = sampler.decode(latent, context, t_enc,
                                        unconditional_guidance_scale=scale,
                                        unconditional_conditioning=uc)
...
def replace_object(sampler, model, tgt_img,
    ...
    ref_latent = model.get_first_stage_encoding(model.encode_first_stage(tgt_img))
