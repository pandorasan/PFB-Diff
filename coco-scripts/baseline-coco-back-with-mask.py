...
from common import parser, load_model, baseline, get_coco_mask, \
...
out_dir = os.path.join(opt.out_dir, 'diffedit-coco-back-with-mask-ratio=1')
os.makedirs(out_dir, exist_ok=True)

img_list = make_dataset_txt('back_list.txt')
with open('back_data.pkl', 'rb') as f:
    data = pickle.load(f)
    ...
    with precision_scope(device.type):
        with model.ema_scope():
            for name in img_list:
                ...
                tgt_mask = 1 - get_coco_mask(name).squeeze()
