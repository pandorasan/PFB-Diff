...
out_dir = os.path.join(opt.out_dir, 'our-baseline-back-coco')
os.makedirs(out_dir, exist_ok=True)

img_list = make_dataset_txt('data/coco/back_list.txt')
with open('data/coco/back_data_new.pkl', 'rb') as f:
    data = pickle.load(f)
    ...
    with precision_scope(device.type):
        with model.ema_scope():
            for name in img_list:
                ...
                tgt_mask = (1 - get_coco_mask_dilate(name, root_dir, [36, 1])).squeeze()
